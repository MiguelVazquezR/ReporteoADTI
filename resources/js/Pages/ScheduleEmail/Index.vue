    ...
    <PublicLayout title="Reportes programados">
        <main class="lg:py-10 lg:px-14">
            <section class="flex items-center justify-between mx-14">
                ...
                <PrimaryButton @click="$inertia.visit(route('schedule-email-settings.create'))">Crear nuevo</PrimaryButton>
            </section>
            <section class="flex flex-col items-center">
                <div class="lg:flex justify-between mb-2 mt-6">
                    <el-dropdown :disabled="disableMassiveActions" @command="handleCommand">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item command="toggle-status">Editar</el-dropdown-item>
                                <el-dropdown-item command="delete">Eliminar</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
                    </el-dropdown>
                </div>
                <el-table :data="schedules" @row-click="handleRowClick" max-height="520" style="width: 90%; margin-top: 30px"
                    ...
                    <el-table-column prop="cco" label="Con copia a">
                        <template #default="scope">
                            <p v-for="(item, index) in scope.row.cco" :key="index" class="bg-primarylight text-primary rounded-md px-2 py-1 text-xs">
                                {{ item }}
                            </p>
                        </template>
                    </el-table-column>
